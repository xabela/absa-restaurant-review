...
alpha_file = open('alphaPickle', 'rb')      
alpha = pickle.load(alpha_file)

bias_file = open('biasPickle', 'rb')      
...
def getAspectSentimentTest(kernel_uji):
    aspect = []
    y_pred_ambience = testingAspectOneData(alpha['ambience'], bias['ambience'], kernel_uji, y_train_ambience)
    y_pred_drink = testingAspectOneData(alpha['drink'], bias['drink'], kernel_uji, y_train_drink)
    y_pred_food = testingAspectOneData(alpha['food'], bias['food'], kernel_uji, y_train_food)
    y_pred_restaurant = testingAspectOneData(alpha['restaurant'], bias['restaurant'], kernel_uji, y_train_restaurant)
    y_pred_service = testingAspectOneData(alpha['service'], bias['service'], kernel_uji, y_train_service)
    ...
    if (1 not in aspect):
        ...
        sentiment_aspect = testingSentimentOneData(alpha['sentiment'], bias['sentiment'], kernel_uji, y_train_sentiment)
